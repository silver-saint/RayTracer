                                    ...
                                    const D3D12_HEAP_PROPERTIES& heapProps)
{
  ...
  HRESULT heapProps = (m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc,
                                                  initState, nullptr, IID_PPV_ARGS(&pBuffer)));
  if (FAILED((m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc,
      initState, nullptr, IID_PPV_ARGS(&pBuffer)))))
  {
      PrintError(L"Couldn't create heapProps", DEBUGLAYER);
  }
  return pBuffer;
}
...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
  ...
  if (!pCompiler)
  {
      ...
      {
          PrintError(L"Couldn't create Dxc compiler Instance", DEBUGLAYER);
      }
      if (FAILED(DxcCreateInstance(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void**)&pLibrary)))
      {
          PrintError(L"Couldn't create Dxc library Instance", DEBUGLAYER);

      }
      if (FAILED(pLibrary->CreateIncludeHandler(&dxcIncludeHandler)))
      {
          PrintError(L"Couldn't create Dxc include handler", DEBUGLAYER);
  ...
  {
      PrintError(L"Failed to create shader file.", DEBUGLAYER);
  ...
  {
      PrintError(L"Failed to create blob with encoding.", DEBUGLAYER);

  }

  // Compile
  IDxcOperationResult* pResult;
  if (FAILED(pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0,
      dxcIncludeHandler, &pResult)))
  {
      PrintError(L"Failed to compile compiler.", DEBUGLAYER);
  ...
  {
      PrintError(L"Failed to get status code.", DEBUGLAYER);
  ...
  if (FAILED(resultCode))
  {
    ...
    if (FAILED(hr))
    {
        PrintError(L"Failed to get shader compiler.", DEBUGLAYER);
    }

    // Convert error blob to a string
    std::vector<wchar> infoLog(pError->GetBufferSize() + 1);
    memcpy(infoLog.data(), pError->GetBufferPointer(), pError->GetBufferSize());
    infoLog[pError->GetBufferSize()] = 0;

    std::wstring errorMsg = L"Shader Compiler Error:\n";
    errorMsg.append(infoLog.data());

    OutputDebugString(errorMsg.c_str());
    PrintError(L"Failed to compile shader file.", DEBUGLAYER);
  ...
  {
      PrintError(L"Failed to get result.", DEBUGLAYER);
  }
  return pBlob;
}
                                           ...
                                           D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
{
  ...
  if (FAILED(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pHeap))))
  {
      PrintError(L"Failed to create descriptor Heap.", DEBUGLAYER);
