...
namespace engine
{
	...
	void VkDeviceCtx::CreateImage(ui32 width, ui32 height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags imageFlag, VkMemoryPropertyFlags memPropertyFlags, VkImage& img, VkDeviceMemory& memDevice)
	{
		...
		if (vkAllocateMemory(device, &allocInfo, nullptr, &memDevice) != VK_SUCCESS) {
			throw std::runtime_error("failed to allocate image memory!");
		}

		vkBindImageMemory(device, img, memDevice, 0);
	}

	void VkDeviceCtx::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
	...
	void VkDeviceCtx::CreateLogicalDevice()
	{
		...
		const std::set<uint32_t> const uniqueFamilies = { indices.graphicsFamily.value(), indices.presentFamily.value() };
		...
		createInfo.queueCreateInfoCount    = static_cast<uint32_t>(deviceExtensions.size());
